...
class UsuarioForm(forms.ModelForm):
    ...
    groups = forms.ModelMultipleChoiceField(
        ...
        error_messages={
            "required": "Selecione pelo menos um grupo de acesso."
        },
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk:
            self.fields["pessoa"].queryset = Pessoa.objects.filter(
                pk=self.instance.pessoa.pk
            )
            self.fields["pessoa_search"].initial = (
                f"{self.instance.pessoa.nome} - {self.instance.pessoa.doc}"
            )
            ...
            try:
                ...
                if hasattr(pessoa, "usuario") and (
                    ...
                    raise forms.ValidationError(
                        "Esta pessoa já possui um usuário vinculado."
                    )
                return pessoa
    ...
    def clean(self):
        ...
        if not groups:
            raise forms.ValidationError(
                "Selecione pelo menos um grupo de acesso."
            )
